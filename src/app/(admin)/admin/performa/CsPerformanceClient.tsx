...
import type { CsPerformanceResult } from "@/lib/cs-performance";
...
type Response = { range: Range; current: CsPerformanceResult; previous: CsPerformanceResult };
...
function Dashboard({ data }: { data: Response }) {
  ...
  return <div className="space-y-4">
    ...
    <section className="overflow-hidden rounded-xl border border-border/60 bg-white"><div className="p-5"><h2 className="font-serif text-lg font-semibold text-text">Riwayat Order Saya</h2><p className="mt-1 text-xs text-text-muted">20 order terbaru pada periode terpilih.</p></div><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-sm"><thead className="bg-surface/50 text-left text-xs font-semibold uppercase tracking-wider text-text-muted"><tr><th className="px-5 py-3">Tanggal</th><th className="px-4 py-3">Nomor Order</th><th className="px-4 py-3">Pelanggan</th><th className="px-4 py-3">Tipe</th><th className="px-4 py-3">Nilai</th><th className="px-5 py-3">Status</th></tr></thead><tbody className="divide-y divide-border/30">{current.recentOrders.map(order => <tr key={order.id}><td className="px-5 py-3.5 tabular-nums text-text-muted">{new Date(order.created_at).toLocaleDateString("id-ID", { timeZone: "Asia/Jakarta" })}</td><td className="px-4 py-3 font-medium text-text">{order.order_number}</td><td className="px-4 py-3">{order.customer_name}</td><td className="px-4 py-3">{order.type === "sell" ? "Jual" : "Buyback"}</td><td className="px-4 py-3 tabular-nums">{formatRupiah(order.total)}</td><td className="px-5 py-3"><StatusBadge tone={order.status === "completed" ? "green" : "red"}>{order.status === "completed" ? "Selesai" : "Dibatalkan"}</StatusBadge></td></tr>)}{!current.recentOrders.length && <tr><td colSpan={6} className="px-5 py-10 text-center text-text-muted">Belum ada order pada periode ini.</td></tr>}</tbody></table></div></section>
    <div className="rounded-xl border border-gold/25 bg-gold/5 p-4 text-xs leading-relaxed text-gold-dark"><strong>Catatan penghasilan:</strong> nominal penghasilan belum ditampilkan karena aturan tarif per CS belum dikonfigurasi. Statistik ini tidak menyamakan omzet atau GP dengan pendapatan pribadi.</div>
  </div>;
}
