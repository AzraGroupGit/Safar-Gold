  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div><h1 className="font-serif text-2xl font-bold text-text">Pelanggan</h1><p className="mt-1 text-sm text-text-muted">{filtered.length} pelanggan</p></div>
      </div>
      ...
      <div className="mb-4">
        <div className="relative max-w-sm">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Cari nama, No WA, atau NIK..." className="w-full rounded-xl border border-border/60 bg-white pl-10 pr-4 py-2.5 text-sm text-text focus:border-gold focus:outline-none" />
        </div>
      </div>
          ...
          <tbody className="divide-y divide-border/30">
            {filtered.map(c => (
              <tr key={c.id} className="hover:bg-surface/30">
                ...
                <td className="px-4 py-3.5 text-right text-sm font-semibold text-text md:px-6">{formatRupiah(c.total_spent)}</td>
          ...
          <div className="relative w-full max-w-2xl rounded-2xl border border-border/60 bg-white shadow-2xl">
            ...
            <div className="p-6 space-y-5">
              <div className="grid gap-3 sm:grid-cols-2">
                {detail.customer.nik && <div className="rounded-xl border border-border/30 bg-surface p-3"><p className="text-[11px] uppercase tracking-wider text-text-muted">NIK</p><p className="mt-1 text-sm font-medium text-text">{detail.customer.nik}</p></div>}
                <div className="rounded-xl border border-border/30 bg-surface p-3"><p className="text-[11px] uppercase tracking-wider text-text-muted">Sumber</p><p className="mt-1 text-sm font-medium text-text">{detail.customer.source ?? "-"}</p></div>
                {detail.customer.instagram && <div className="rounded-xl border border-border/30 bg-surface p-3"><p className="text-[11px] uppercase tracking-wider text-text-muted">Instagram</p><p className="mt-1 text-sm font-medium text-text">{detail.customer.instagram}</p></div>}
                    ...
                    <tbody className="divide-y divide-border/20">
                      {detail.orders.map(o => (
                        <tr key={o.id}>
                          ...
                          <td className="py-2.5 text-right font-semibold">{formatRupiah(o.total)}</td>
            ...
            <div className="flex justify-end border-t border-border/40 bg-surface/30 px-6 py-4 rounded-b-2xl">
              <button onClick={() => setDetail(null)} className="rounded-xl border border-border/60 px-5 py-2.5 text-sm font-medium text-text-muted hover:bg-white">Tutup</button>
            </div>
          </div>
        </div>
      )}
      ...
      {invoiceOrder && (
        <div className="fixed inset-0 z-[60] flex items-start justify-center overflow-y-auto px-4 pt-[6vh] pb-10">
          ...
          <div className="relative w-full max-w-2xl rounded-2xl border border-border/60 bg-white shadow-2xl">
            ...
            <div className="flex items-center justify-between border-t border-border/40 bg-surface/30 px-6 py-4 rounded-b-2xl">
              <button onClick={backToDetail} className="rounded-xl border border-border/60 px-4 py-2.5 text-sm font-medium text-text-muted hover:bg-white">← Kembali</button>
              <button onClick={() => setInvoiceOrder(null)} className="rounded-xl border border-border/60 px-5 py-2.5 text-sm font-medium text-text-muted hover:bg-white">Tutup</button>
            </div>
          </div>
        </div>
      )}
