...
type StockRow = { gold_type_id: string; qty: number; min_qty: number; updated_at: string; gold_types: { name: string; weight: number | null } | null };
...
export default function StockClient() {
  ...
  async function handleAdjust() {
    ...
    if (data.success) { setShowModal(false); setAdjProduct(""); setAdjQty(1); setAdjNotes(""); fetchData(); } else { setError(data.error ?? "Gagal"); }
    setSaving(false);
  }
  ...
  async function handleSaveMinQty() {
    ...
    setSaving(true);
    await fetch("/api/admin/update-settings", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ settings: { [`stock_min_${minProduct.gold_type_id}`]: String(minQty) } }) });
  ...
  const filteredMovements = useMemo(() => {
    ...
    return movements.filter(m => m.type === moveFilter);
  ...
  return (
    <div>
      <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
        <div><h1 className="font-serif text-2xl font-bold text-text">Stok</h1><p className="mt-1 text-sm text-text-muted">Inventori Logam Mulia (LM)</p></div>
        <button onClick={() => { setAdjProduct(stock[0]?.gold_type_id ?? ""); setAdjType("in"); setAdjQty(1); setAdjNotes(""); setShowModal(true); }} className="rounded-xl border-2 border-gold/40 px-5 py-2.5 text-sm font-semibold text-gold-dark transition-all hover:border-gold hover:bg-gold/5">Sesuaikan Stok</button>
      </div>
          ...
          <tbody className="divide-y divide-border/30">
            {filteredStock.map(s => (
              <tr key={s.gold_type_id} className="hover:bg-surface/30">
                <td className="px-4 py-3.5 md:px-6"><p className="text-sm font-medium text-text">{s.gold_types?.name ?? s.gold_type_id}</p></td>
                ...
                <td className="px-4 py-3.5 text-center"><span className={`text-sm font-bold ${s.qty <= s.min_qty ? "text-red-500" : "text-text"}`}>{s.qty}</span></td>
      ...
      <div className="mb-4 flex items-center justify-between">
        <h3 className="font-serif text-lg font-semibold text-text">Riwayat Pergerakan Stok</h3>
        <div className="flex gap-1 rounded-xl border border-border/60 bg-white p-1">
          {[{ key: "all", label: "Semua" }, { key: "in", label: "Masuk" }, { key: "out", label: "Keluar" }].map(f => (
            <button key={f.key} onClick={() => setMoveFilter(f.key as any)} className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${moveFilter===f.key?"bg-gold/10 text-gold-dark":"text-text-muted hover:text-text"}`}>{f.label}</button>
          ))}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center px-4 pt-[20vh]">
          ...
          <div className="relative w-full max-w-sm rounded-2xl border border-border/60 bg-white p-6 shadow-2xl">
            ...
            <div className="space-y-4">
              <div><label className="mb-1 block text-xs font-medium text-text-muted">Produk</label><select value={adjProduct} onChange={e => setAdjProduct(e.target.value)} className="w-full rounded-lg border border-border/60 bg-white px-3 py-2.5 text-sm">{stock.map(s => <option key={s.gold_type_id} value={s.gold_type_id}>{s.gold_types?.name ?? s.gold_type_id} ({s.qty})</option>)}</select></div>
              <div><label className="mb-1 block text-xs font-medium text-text-muted">Tipe</label><div className="flex gap-3"><button onClick={() => setAdjType("in")} className={`flex-1 rounded-lg border px-4 py-2 text-sm font-medium ${adjType==="in"?"border-emerald-400 bg-emerald-50 text-emerald-700":"border-border/60 text-text-muted"}`}>Masuk (+)</button><button onClick={() => setAdjType("out")} className={`flex-1 rounded-lg border px-4 py-2 text-sm font-medium ${adjType==="out"?"border-red-400 bg-red-50 text-red-600":"border-border/60 text-text-muted"}`}>Keluar (−)</button></div></div>
            ...
            <div className="mt-6 flex gap-3"><button onClick={() => setShowModal(false)} className="flex-1 rounded-xl border border-border/60 px-4 py-2.5 text-sm font-medium text-text-muted hover:bg-surface">Batal</button><button onClick={handleAdjust} disabled={saving} className="gold-gradient-bg flex-1 rounded-xl px-4 py-2.5 text-sm font-semibold text-white shadow-sm disabled:opacity-60">{saving ? "..." : "Simpan"}</button></div>
          </div>
        </div>
      )}
      ...
      {showMinModal && minProduct && (
        <div className="fixed inset-0 z-50 flex items-start justify-center px-4 pt-[25vh]">
          ...
          <div className="relative w-full max-w-xs rounded-2xl border border-border/60 bg-white p-6 shadow-2xl">
            ...
            <div className="mt-6 flex gap-3"><button onClick={() => setShowMinModal(false)} className="flex-1 rounded-xl border border-border/60 px-4 py-2.5 text-sm font-medium text-text-muted hover:bg-surface">Batal</button><button onClick={handleSaveMinQty} disabled={saving} className="gold-gradient-bg flex-1 rounded-xl px-4 py-2.5 text-sm font-semibold text-white shadow-sm disabled:opacity-60">{saving ? "..." : "Simpan"}</button></div>
          </div>
        </div>
      )}
    </div>
