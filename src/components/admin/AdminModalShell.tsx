...
export default function AdminModalShell({ title, description, eyebrow, onClose, children, footer, size = "wide", accent = "gold" }: {
  title: string;
  description: string;
  eyebrow: string;
  onClose: () => void;
  ...
  const titleId = `admin-modal-${title.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`;
  const accentClasses = accent === "danger" ? "bg-red-500" : "bg-gold";
  const eyebrowClasses = accent === "danger" ? "text-red-700" : "text-gold-dark";
  ...
  return <div className="fixed inset-0 z-[60] flex items-center justify-center overflow-y-auto p-4 sm:p-6">
    ...
    <section ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby={titleId} className={`relative my-auto flex max-h-[calc(100dvh-2rem)] w-full flex-col overflow-hidden rounded-xl border border-border/60 bg-white shadow-[0_24px_80px_rgba(34,28,16,.22)] ${size === "wide" ? "max-w-4xl" : "max-w-lg"}`}>
      <header className="relative flex shrink-0 items-start justify-between gap-5 border-b border-border/40 px-5 py-4 sm:px-6 sm:py-5"><span aria-hidden="true" className={`absolute inset-x-0 top-0 h-0.5 ${accentClasses}`} /><div className="min-w-0"><p className={`text-[10px] font-semibold uppercase tracking-[.16em] ${eyebrowClasses}`}>{eyebrow}</p><h2 id={titleId} className="mt-1 font-serif text-xl font-semibold text-text">{title}</h2><p className="mt-1 max-w-2xl text-xs leading-relaxed text-text-muted sm:text-sm">{description}</p></div><button ref={closeRef} type="button" aria-label={`Tutup ${title}`} onClick={onClose} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-border/50 text-lg text-text-muted transition-colors hover:bg-surface hover:text-text focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold/40">×</button></header>
