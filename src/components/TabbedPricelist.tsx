...
import { formatRupiah, BB_LM_ORDER } from "@/lib/gold-api";
...
export default function TabbedPricelist({
  ...
  const filtered = prices
    ...
    .sort((a, b) => {
      if (active === "bb-perhiasan") return (b.karat ?? 0) - (a.karat ?? 0);
      if (active === "bb-lm") {
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border/40 bg-surface/30 text-left text-xs font-semibold uppercase tracking-wider text-text-muted">
                ...
                <th className="px-5 py-4 md:px-8">Harga per Gram</th>
                {isLM && <th className="px-5 py-4 md:px-8">Harga Total</th>}
              </tr>
            </thead>
